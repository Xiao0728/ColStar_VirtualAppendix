...
class ColBERT(ElectraPreTrainedModel):
...
#             self.tokenizer = BertTokenizerFast.from_pretrained('prajjwal1/bert-medium')
    ...
    def query(self, input_ids, attention_mask):
        ...
        Q = self.electra(input_ids, attention_mask=attention_mask)[0]
        Q = self.linear(Q)
    ...
    def doc(self, input_ids, attention_mask, keep_dims=True):
        input_ids, attention_mask = input_ids.to(DEVICE), attention_mask.to(DEVICE)
        D = self.electra(input_ids, attention_mask=attention_mask)[0]
        D = self.linear(D)
